...
class IntegralFrame(SpacecraftFrame):
    """
    The integral spacecraft frame in azimuth and elevation.  The frame is defined
    as a quaternion that represents a rotation from the integral frame to the ICRS
    ...
    Example use:
        ...
        >>> integral_frame = IntegralFrame(quaternion=quat)
        >>> coord = SkyCoord(100.0, -30.0, unit='deg')
        >>> az_el = SkyCoord.transform_to(integral_frame)
    """
    pass
...
def icrs_to_integral(icrs_frame, integral_frame):
    ...
    Returns:
        (:class:`IntegralFrame`)
    """
    return icrs_to_spacecraft(icrs_frame, integral_frame)
