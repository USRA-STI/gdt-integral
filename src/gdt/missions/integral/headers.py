...
class SpiHeader(Header):
    ...
    def __setitem__(self, key, val):
        if not isinstance(key, tuple):
            if key.upper() == 'TSTART':
                self['DATE-OBS'] = Time(val, format='spi').iso 
            elif key.upper() == 'TSTOP':
                self['DATE-END'] = Time(val, format='spi').iso
            else:
                pass
...
_mjdreff_card = ('MJDREF', _mjdreff, 'MJD of GLAST reference epoch, fractional part')
...
_trigtime_card = ('TIMEZERO', 0.0, 'Trigger time relative to MJDREF, double precision')
...
class Spi_acsSecondaryHeader(Spi_acsPrimaryHeader):
    ...
    keywords = [ _telescope_card, _instrument_card, _date_obs_card,
                _date_end_card, _timesys_card, _timeunit_card,
...
class SPIOrbitDataHeader(SPIOrbitPrimaryHeader):
    ...
    keywords = [_extname_card, _extrel_card, _telescope_card, _instrument_card, 
                ...
                _sw_type_card, _swbound_card, _bcppid_card, _PREVSWI_card, _OBTSTART_card, _OBTEND_card]
